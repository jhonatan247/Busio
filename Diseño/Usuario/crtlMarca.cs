...
{
    public partial class crtlMarca : UserControl
    {
        long Id = 0;
        public crtlMarca()
        {
        ...
        private void bnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (Id > 0)
            {
                try
                {
                    if (new Marca(Id, txNombre.Text, txDescripcion.Text, true).Update() > 0)
                    {
            ...
            else
            {
                try
                {
                    if (new Marca(txNombre.Text, txDescripcion.Text).Insert() > 0)
                    {
        ...
        private void bnBorrar_Click(object sender, EventArgs e)
        {
            if (Id > 0)
            {
                ...
                {
                    if (new Marca(Id, false).Update_Estado() > 0)
                    {
                        Restablecer();
                        return;
                    }
        ...
        private void bnPapelera_Click(object sender, EventArgs e)
        {
            ...
            new frmReciclajeMarca().ShowDialog();
            crtlBuscar.Buscar();
            try
            {
