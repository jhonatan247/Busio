...
{
    public partial class crtlBuscarCliente : UserControl
    {
        bool accion = true;
        ...
        public void Buscar()
        {
            ...
            {
                cbBuscarPor.Items.Remove(cbiSucursal); primero = false;
            }
            if (accion)
            {
        ...
        private void dataGridViewX1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (!multiSelect)
            {
                ...
                if (id != 0)
                {
                    try
                    {
                        DataRow dRow = new Producto().SelectVista(new Sucursal().SelectId(AccionesSistema.ObtenerValorSelecion(dtBuscar, 1).ToString()), id);
                        try
                        {
