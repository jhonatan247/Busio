...
{
    public partial class crtlCategoria : UserControl
    {
        long Id = 0;
        public crtlCategoria()
        {
            InitializeComponent();
        }
        public void Restablecer()
        {
            lbMarca.Text = "NUEVA CATEGORÍA";
            txDescripcion.Clear();
            txNombre.Clear();
        ...
        private void bnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (Id > 0)
            {
                try
                {
                    if (new Categoria(Id, txNombre.Text, txDescripcion.Text, true).Update() > 0)
                    {
            ...
            else
            {
                try
                {
                    if (new Categoria(txNombre.Text, txDescripcion.Text).Insert() > 0)
                    {
        ...
        private void bnBorrar_Click(object sender, EventArgs e)
        {
            if (Id > 0)
            {
                if (MessageBoxEx.Show("La Categoría seleccionada será enviada a la papelera y los productos relacionados dejarán de mostrarse.\n¿Deseas continuar?", "Reciclar",
                    MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    if (new Categoria(Id, false).Update_Estado() > 0)
                    {
                        Restablecer();
                        return;
                    }
        ...
        private void bnPapelera_Click(object sender, EventArgs e)
        {
            ...
            new frmReciclajeCategoria().ShowDialog();
            crtlBuscar.Buscar();
            try
            {
